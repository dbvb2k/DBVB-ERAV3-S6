...
def print_training_config(use_augmentation, initial_lr):
    ...
    print(f"LR Schedule: Step decay with gamma={0.1} at step={15}")
...
def train(use_augmentation=True):
    ...
    for epoch in range(20):
        print(f"\nEpoch {epoch+1}/20 (lr={scheduler.get_last_lr()[0]:.6f}):")
        
        train_loss, train_accuracy = train_epoch(model, train_loader, optimizer, criterion, device, scheduler)
